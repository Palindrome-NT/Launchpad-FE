...
import { useUpdatePostMutation, useUploadMediaMutation } from '../../lib/store/api/postsApi';
...
const EditPostForm: React.FC<EditPostFormProps> = ({ post, onClose, onSuccess }) => {
  const [updatePost, { isLoading: isUpdating }] = useUpdatePostMutation();
  const [uploadMedia, { isLoading: isUploading }] = useUploadMediaMutation();
  const [mediaFiles, setMediaFiles] = useState<File[]>([]);
  const [mediaUrls, setMediaUrls] = useState<string[]>(post.media || []);
  const [mediaTypes, setMediaTypes] = useState<('image' | 'video')[]>(post.mediaType || []);
            ...
            <Form className="space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-6 border-t">
                ...
                <Button
                  type="submit"
                  loading={isUpdating}
                  disabled={!values.content.trim() || isUpdating}
                  className="px-6"
                >
